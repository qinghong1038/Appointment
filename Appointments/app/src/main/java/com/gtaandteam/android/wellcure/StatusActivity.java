...
public class StatusActivity extends AppCompatActivity {
    ...
    private void addAppointmentToDatabase()
    {
        ...
        UserDb1 = FirebaseDatabase.getInstance().getReference("appointmentDB");
        UserDb1.child(FbAuth.getCurrentUser().getUid()).push().setValue(newAppointment).addOnCompleteListener(this, new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task)
            {
                ...
                {
                    Log.d(LOG_TAG, "Stored New Appointment to Database");
